...
$(function ()
{
    ...
    var viewModel,
    dataSource = {/*there should be a reference to a controller in here*/ },
    targetSelector = "#jqdt",
    queryUrl = "",
    ...
    jqdt,
    //no caps allowed for some reason
    //tableElementName = "jquery-datatable",
    viewModel,
    jQueryDataTableControl,
    jQueryDataTable,
    scopeSelector = "#frmWorkload";

    setupCSS();
    //setupTable();
    //viewModel = DataTablesHelper.setupViewModel(targetSelector, queryUrl, queryCallback, getColumnsWorkloadPriority(), "calc(70vh)");

    //jqdt = document.createElement("table", { is: tableElementName });
    //document.body.appendChild(jqdt);

    jqdt = DataTablesHelper.getCustomControl("", function () { }, getColumnsWorkloadPriority(), "100%");
    console.log("viewModel should have a reference now, but I'm not sure how to get it from the helper", viewModel);

    document.querySelector(scopeSelector).appendChild(jqdt);
    console.log("jqdt", jqdt);
